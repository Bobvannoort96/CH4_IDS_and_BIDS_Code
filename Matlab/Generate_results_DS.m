...
%% For ordinary DS (k=4)
type_of_DS = 'D';
partitions_DS = ["P0" "P1" "P2" "P3" "P4" "P99"];
...
% for factor = [2.8 ]
    ...
    ylabel('t2')
    zlabel('t3')
    title({append("Misclosure Space Partitioning at R = ", num2str(factor)), append('Type ', type_of_DS, ' datasnooping')} )
    legend(h,p_name,'Interpreter','latex','box','on','FontSize',14)
    axis equal
    axis 'off'
    ...
    subdirs = fullfile(topDir, 'Figures_report', 'DS', type_of_DS);
    mkdir(subdirs)
    % Print the figure using the '-bestfit' option
    print(gcf, append(subdirs, '\Partitioning_DS_DIA_noaxis_v1_', num2str(factor), '.pdf'), '-dpdf', '-bestfit'); % For saving as a PDF
    
    view(135, 30);
    % turn of legend for this one
    legend('off')
    % Print the figure using the '-bestfit' option
    print(gcf, append(subdirs, '\Partitioning_DS_DIA_noaxis_v2_', num2str(factor), '.pdf'), '-dpdf', '-bestfit'); % For saving as a PDF
